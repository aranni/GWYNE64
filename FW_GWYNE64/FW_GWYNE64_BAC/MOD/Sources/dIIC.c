...
void IIC_Init (void)
{
  ...
  SYS_ConfigParam.RunConfig |= CFG_AUX_RUNSTATE_MASK;   // comienzo habilitando periféricos
	
  bResult = IIC_InitReceiveMsg (IIC_RTC_ADDRESS, 2);    // Leer el estado del oscilador
  if (bResult != OK){    
    SYS_ConfigParam.RunConfig &= ~CFG_AUX_RUNSTATE_MASK;  // Si falla deshabilitar perifericos aux.
    return;
  }
  ...
  if ( (!(RxPacket[0] & 0x80)) &&       // Si estaba habilitado CH=0
       (!(SYS_ConfigParam.RunConfig & CFG_RTC_RUNSTATE_MASK)) ){
          ...
          bResult = IIC_InitSendMsg (IIC_RTC_ADDRESS, 2, 0);
          if (bResult != OK){    
            SYS_ConfigParam.RunConfig &= ~CFG_AUX_RUNSTATE_MASK;  // Si falla deshabilitar perifericos aux.
            return;
          }
  ...
  bResult = IIC_InitSendMsg (IIC_DAC_ADDRESS, 2, 0);
  if (bResult != OK){    
    SYS_ConfigParam.RunConfig &= ~CFG_AUX_RUNSTATE_MASK;  // Si falla deshabilitar perifericos aux.
    return;
  }
...
void IIC_WriteRTCMem (UINT8 Len, UINT8 Addr, UINT8* DataPtr) {
  ...
  if (!(SYS_ConfigParam.RunConfig & CFG_AUX_RUNSTATE_MASK))  
    return;
 TxPacket[0] = Addr;
...
//---------------------------------------------------------------
// Send y Receive para usar en instancia de inicialización.
//
UINT8 IIC_InitSendMsg (UINT8 SlaveAddress, UINT8 LenTx, UINT8 LenRx)
{
...
UINT8 IIC_InitReceiveMsg (UINT8 SlaveAddress, UINT8 LenRx)
{
	MasterRxFlag = TRUE;
	RxCompleteflag = FALSE;
	RxPacketpositionptr = &RxPacket[0];			// apuntar al primer elemento de RxPacket
