...
//        (*(volatile unsigned char *) 0x000A) = 0x0C;  // PEAR   = ((LSTRE|RDWE)&~NECLK)
//        (*(volatile unsigned char *) 0x000E) = 0x01;  // EBICTL = Enable clock stretching (ESTR)
//        (*(volatile unsigned char *) 0x0013) = 0x07;  // MISC   = (EXSTR1+EXSTR0+ROMHM+ROMON)
//        (*(volatile unsigned char *) 0x000B) = 0xE2;  // MODE   = (MODC+MODB+MODA+EMK+!EME)
            //  about 20.83MHz - external bus is 16MHz rated (10bT only)
