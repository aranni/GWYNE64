...
INT8 tcp_changemtu  (INT8 sochandle, UINT16 rmtu)
{
	struct tcb* soc;
...
INT8 tcp_getsocket (UINT8 soctype, UINT16 tout, INT32 (*listener)(INT8, UINT8, UINT32, UINT32), UINT16 rmtu )
{
	INT8 i;
	struct tcb* soc;
...
//  ABR: si hay problemas con https dinamicos, impedir que se cambie el tamaño del paquete..  
...
/* Retransmit timer	*/
// ABR: 03-MAR-2009 .. 
// Según reporte de bug: [ 1246424 ] retransmission timer bug / [ 790804 ] bug in tcp_send
	init_timer((UINT8)(soc->retransmit_timerh), (TCP_DEF_RETRY_TOUT*TIMERTIC)); //(CfgMsgInfo.var_TCP_DEF_RETRY_TOUT)

/* KeepAlive timer	*/
// ABR: 03-MAR-2009 .. Según reporte de bug: [ 790804 ] bug in tcp_send
	if(soc->state == TCP_STATE_CONNECTED)
		...
		switch(soc->state) {
			...
			case TCP_STATE_CONNECTED:
				...
				*/
				...
				soc->retries_left--;
//				init_timer((UINT8)(soc->retransmit_timerh), ((CfgMsgInfo.var_TCP_DEF_RETRY_TOUT)*TIMERTIC));
...
//						init_timer((UINT8)(soc->retransmit_timerh), ((CfgMsgInfo.var_TCP_DEF_RETRY_TOUT)*TIMERTIC));
			...
			case TCP_STATE_TIMED_WAIT:
				...
				if(soc->retries_left) {
					soc->retries_left--;
					init_timer((UINT8)(soc->retransmit_timerh), (TCP_DEF_RETRY_TOUT*TIMERTIC));
//					init_timer((UINT8)(soc->retransmit_timerh), ((CfgMsgInfo.var_TCP_DEF_RETRY_TOUT));
					break;
				}
		  ...
		  //soc->receive_mtu = 0;
			...
			if( received_tcp_packet.hlen_flags & TCP_FLAG_SYN )	{
				...
				if( received_tcp_packet.hlen_flags & TCP_FLAG_ACK )	{
				 ...
				 // ABR_TEST:2014-04-18 : comentado el siguiente return(0)
				 // https://groups.google.com/forum/#!topic/comp.arch.embedded/DilrtOyrFrc
				 return(0);				
			...
			if( soc->send_unacked != soc->send_next ) {
				...
				if( received_tcp_packet.ackno == soc->send_next ) {
				  ...
				  // ABR_TEST: cancelado el evento ACK al handler porque no se esta usando....2014-04-01
			...
			if(soc->receive_next != received_tcp_packet.seqno)
			{
				...
				tcp_sendcontrol(sochandle);			
				return(0);				
        //soc->receive_next = received_tcp_packet.seqno;  // ABR_TEST
...
INT16 process_tcp_out (INT8 sockethandle, UINT8* buf, UINT16 blen, UINT16 dlen)
{
	...
	*buf++ = (UINT8)(soc->receive_mtu >> 8);
	*buf++ = (UINT8)(soc->receive_mtu & 0xFF);
	*buf++ = 0;								/* Checksum	*/
	*buf++ = 0;
	*buf++ = 0;								/* Urgent	*/
	*buf++ = 0;
...
// ABR-11-MAR-2010 (Necochea TTL de 100 a 128)//	(
...
{
	soc->state = nstate;
	soc->retries_left = TCP_DEF_RETRIES; //CfgMsgInfo.var_TCP_DEF_RETRIES;

	/* Retransmit timer	*/
	// ABR: 02-MAR-2009 .. 
// Comentado según reporte de bug: [ 1246424 ] retransmission timer bug	
//	init_timer((UINT8)(soc->retransmit_timerh), (CfgMsgInfo.var_TCP_DEF_RETRY_TOUT));
	...
	switch(soc->state) {
		...
		case TCP_STATE_SYN_SENT:	
			...
			//init_timer((UINT8)(soc->retransmit_timerh), (CfgMsgInfo.var_TCP_INIT_RETRY_TOUT));
...
/* Retransmit timer	*/
//  ABR: 02-MAR-2009 .. debería estar antes del switch? porque se cambia si es TCP_STATE_SYN_SENT
//	init_timer((UINT8)(soc->retransmit_timerh), (CfgMsgInfo.var_TCP_DEF_RETRY_TOUT));
//	init_timer((UINT8)(soc->retransmit_timerh), ((CfgMsgInfo.var_TCP_DEF_RETRY_TOUT)*TIMERTIC));
