...
#include "wAjaxEvents.h"

extern struct http_server_state https [NO_OF_HTTP_SESSIONS];

void EventsAjax(BYTE fat_index, UINT16 wLen, BYTE bSession) 
{
...
//  wLen++;				//not used - stops compiler warning
    ...
    if (TagFlags == 1) 
    {
        ...
        wLogIdx = FLA_ReadLogEvent(pReadLogBuffer);        
      	if ( ERROR != wLogIdx ) 
       	{
        	  ...
        	  (void)sprintf((char*)tmpbuf, "<%s>%02x/%02x/%02x - %02x:%02x:%02x</%s>\n", 
        	                    (char*)"LogDate", 
                        			((LOG_EVENT_FORMAT*)pReadLogBuffer)->LogDateTime.Date,
                        			((LOG_EVENT_FORMAT*)pReadLogBuffer)->LogDateTime.Month,
                        			((LOG_EVENT_FORMAT*)pReadLogBuffer)->LogDateTime.Year,
                        			((LOG_EVENT_FORMAT*)pReadLogBuffer)->LogDateTime.Hour,
                        			((LOG_EVENT_FORMAT*)pReadLogBuffer)->LogDateTime.Minute,
                        			((LOG_EVENT_FORMAT*)pReadLogBuffer)->LogDateTime.Second,
        	                    (char*)"LogDate");
            (void)strcat((char*)sndbuf, (char*)tmpbuf);
            
        	  (void)sprintf((char*)tmpbuf, "<%s>%d</%s>\n", 
        	                    (char*)"EventId", 
        	                    ((LOG_EVENT_FORMAT*)pReadLogBuffer)->EventId, 
        	                    (char*)"EventId");
            (void)strcat((char*)sndbuf, (char*)tmpbuf);

        	  (void)sprintf((char*)tmpbuf, "<%s>%d</%s>\n", 
        	                    (char*)"EventSource", 
        	                    ((LOG_EVENT_FORMAT*)pReadLogBuffer)->EventSource, 
        	                    (char*)"EventSource");
            (void)strcat((char*)sndbuf, (char*)tmpbuf);	 

         	  (void)sprintf((char*)tmpbuf, "<%s>%d</%s>\n", 
        	                    (char*)"EventState", 
        	                    ((LOG_EVENT_FORMAT*)pReadLogBuffer)->EventLastState, 
        	                    (char*)"EventState");
            (void)strcat((char*)sndbuf, (char*)tmpbuf);
            
        	  (void)sprintf((char*)tmpbuf, "<%s>%02x/%02x/%02x - %02x:%02x:%02x</%s>\n", 
        	                    (char*)"EventStateDate", 
                        			((LOG_EVENT_FORMAT*)pReadLogBuffer)->StateDateTime.Date,
                        			((LOG_EVENT_FORMAT*)pReadLogBuffer)->StateDateTime.Month,
                        			((LOG_EVENT_FORMAT*)pReadLogBuffer)->StateDateTime.Year,
                        			((LOG_EVENT_FORMAT*)pReadLogBuffer)->StateDateTime.Hour,
                        			((LOG_EVENT_FORMAT*)pReadLogBuffer)->StateDateTime.Minute,
                        			((LOG_EVENT_FORMAT*)pReadLogBuffer)->StateDateTime.Second,
        	                    (char*)"EventStateDate");
            (void)strcat((char*)sndbuf, (char*)tmpbuf);
...
//            pLogMsg->Data[50] = '\0';
        	  ...
        	  (void)sprintf((char*)tmpbuf, "<%s>%s</%s>\n", 
        	                    (char*)"LogText", 
        	                    (char*)&(((LOG_EVENT_FORMAT*)pReadLogBuffer)->Data[0]), 
//        	                    (char*)pReadLogBuffer,
        	                    (char*)"LogText");
            (void)strcat((char*)sndbuf, (char*)tmpbuf);
...
//          	(void)strcpy((char*)sndbuf, BadHTTP400);
  ...
  if (TagFlags == 10)
  {
    ...
    if (LogParam)
      idx = FLA_ResetLogEventReadCntr(LogParam);
    else
      idx = FLA_ResetLogEventReadCntr(10);
