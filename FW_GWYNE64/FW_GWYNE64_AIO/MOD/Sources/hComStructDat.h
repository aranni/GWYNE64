...
typedef struct {
  ...
  UINT8   RunConfig;
	UINT8 	DebugLevel;       // Habilitacion de debug
...
typedef struct {
	...
	UINT16  AAG_InpWord_A;     // Entrada Analogica 1 como WORD
	UINT16  AAG_InpWord_B;     // Entrada Analogica 2 como WORD
} GIO_STATE_VAL;
...
typedef struct {
  UINT8   MaxMsgCntr;				// Mayor cantidad de mensajes acumulados
	...
	UINT16  WatchRstTmr;      // Causa del ultimo reset
...
typedef struct CFG_FW_VERSION {
        ...
        UINT8       FW_VersionFullName[SIZE_FW_FULL+1];
        UINT8       Reservado[21];
}CFG_FW_VERSION;
...
#define LOG_DATA_SIZE 150  

//--Estructura de formato de LOGs de debug
typedef struct {
...
typedef struct {
    ...
    UINT16     	EventId;
    UINT8     	EventSource;
    UINT8      	EventLastState;
    DATE_TIME 	StateDateTime;   
    UINT16     	EventCode;
    UINT8     	EventParamByteA;
    UINT8     	EventParamByteB;
    UINT16     	EventParamWord;
    UINT32     	EventParamDword;
    UINT8       Data[LOG_DATA_SIZE];    
}LOG_EVENT_FORMAT;
...
#define MAX_UNITELWAY_DATA   (129)  // BYTES
typedef struct {
	UINT8	  DLE_BYTE;				  // = 0
	UINT8	  STX_BYTE;				  // = 0
	UINT8	  ADD_BYTE;				  // = 0
...
//
  ...
  #define MAX_POOLES_QTTY  		  6
  #define MEM_MAX_BUF_LEN     262
 
#elif  (FW_APP_BUILT == FW_APP_SPA)			      // SPACOM_ABB <-> MODBUS_TCP_SLAVE + MODBUS_RTU_SLAVE
  #define MAX_POOLES_QTTY  		  8
  #define MEM_MAX_BUF_LEN     262
 
#elif  (FW_APP_BUILT == FW_APP_BIO)			      // BIOMETRIC_ANVIZ + IO <-> UDP_MESSAGE + LOG_REPORT
  #define MAX_POOLES_QTTY  		 10
  #define MEM_MAX_BUF_LEN     200

#else
    #error Falta implementar aplicacion y definir uso de memoria (ver hComStructDat.h)
#endif
...
//--------------------------------------------------------------------------------------------
//     Estructuras para manipulacion de encuestas periodicas Auto modbus query
//--------------------------------------------------------------------------------------------
//
// -------------------------------------------------------------------------
// Estructura de mensajes para comando y respuesta
//
#define MSG_MAX_AUTOQUERY_LEN   252

#define MAX_AUTOQUERY_BUF     1024    // Cantidad de bytes reservados para alojar respuestas de autoquery.				
#define MAX_AUTOQUERY_BANKS   5			  // Cantidad maxima de bancos a alojar
#define AUTOQUERY_BANKS_INI   0x05    // codigo para estado de bancos inicializados en EEPROM

#define AUTOQUERY_EXTRA_LEN   0x01    // Bytes adicionales reservados en cada banco..

// Definiciones bytes y bits de control de procesos
//
#define   AUTOQUERY_MASK_POL_ENABLE       0x01      // Habilita polling
#define   AUTOQUERY_MASK_RESET_LIST       0x02      // Resetear lista de nodos
#define   AUTOQUERY_MASK_MIRR_FRAME       0x04      // habilita retransmisión por Comm2

#define   AUTOQUERY_MASK_INIT_SEQUENCE    0x10      // comienza una secuencia de polling


typedef struct {
	UINT16    ModbusQueryAddress;   // Direccion de la consulta modbus
	UINT8     ModbusQueryOpCode;    // Tipo de operacion en la encuesta
	UINT8     ModbusQueryLen;       // Cantidad de registros
	UINT16    BankOffsetStart;      // Offset hasta el comienzo del banco en el array
	UINT16    BankModbusAddress;    // Dirección de mapeo del banco en modbus
	UINT8     BankStatus;           // Estado del banco..?	
//	UINT8     BankSize;             // Tamaño del banco en bytes
} AUTOQUERY_BANK_ALLOC;



//--------------------------------------------------------------------------------------------
//     Estructuras para manipulacion de encuestas y respuestas de SYMEO POSITIONING
//--------------------------------------------------------------------------------------------
//
// -------------------------------------------------------------------------
// Estructura de mensajes para comando y respuesta
//
#define MSG_MAX_SYMEO_LEN   50

#define MAX_SYMEO_BUF     512     // Cantidad de bytes reservados para alojar respuestas de autoquery.				
#define MAX_SYMEO_BANKS   5		  // Cantidad maxima de bancos a alojar
#define SYMEO_BANKS_INI   0x05    // codigo para estado de bancos inicializados en EEPROM

#define SYMEO_EXTRA_LEN   0x01    // Bytes adicionales reservados en cada banco..

// Definiciones bytes y bits de control de procesos
//
#define   SYMEO_MASK_POL_ENABLE       0x01      // Habilita polling
#define   SYMEO_MASK_RESET_LIST       0x02      // Resetear lista de nodos
#define   SYMEO_MASK_MIRR_FRAME       0x04      // habilita retransmisión por Comm2

#define   SYMEO_MASK_INIT_SEQUENCE    0x10      // comienza una secuencia de polling


typedef struct {
	UINT16    SourceGroupId;        // Id de grupo del transmisor
	UINT8     SourceStationId;      // Id de estacion del transmisor
	UINT8     SourceBBt;            // Base station Bit del transmisor 1:master 0:slave
	UINT16    TargetGroupId;        // Id de grupo del receptor
	UINT8     TargetStationId;      // Id de estacion del receptor
	UINT8     TargetBBt;            // Base station Bit del receptor 1:master 0:slave
	UINT32    Distance;             // Distancia mm
	UINT32    Velocity;             // Velocidad mm/s
	UINT8     Level;                // Señal en dB
	UINT8     Error;                // error
	UINT8     Status;               // estado
	UINT8     WatchDog;             // byte check
} SYMEO_BANK_ALLOC;


typedef union _SYMEO_LPR_ADDRESS
{
  	UINT16	  StationAddress;
    struct {
    		UINT16	stationID   :5;				// Id de estacion 
    		UINT16  groupID     :10;			// Id de grupo
    		UINT16	BB          :1;				// Base station Bit del transmisor 1:master 0:slave
    }AddrFields;
}SYMEO_LPR_ADDR;
...
typedef struct {
	UINT8	            Start;
	UINT8	            Type;	
	SYMEO_LPR_ADDR		SourceAddr;
	SYMEO_LPR_ADDR		TargetAddr;
	UINT8	            AntenaNumber;
  UINT32            Distance;
  UINT32            Velocity;    
	UINT8	            Level;
