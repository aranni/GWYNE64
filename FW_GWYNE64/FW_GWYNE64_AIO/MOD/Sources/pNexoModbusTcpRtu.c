...
#ifdef   TCP_UNITELWAY_MODE
{EVT_UNITELWAY_REQUEST  , f_NexusUnitelwayTcpAny  },
{EVT2_SCI_RXMSG 	      , f_NexusUnitelwayRtuMsg  },
{EVT2_SCI_RXERR 	      , f_NexusUnitelwayRtuErr  },
{EVT2_SCI_TXERR 	      , f_NexusUnitelwayRtuErr  },
{EVT2_SCI_TXBUSY        , f_NexusUnitelwayRtuErr  },
#endif
...
//-----------------------------------------------------------------------------
// Procesar requerimiento UnitelwayTcp CUALQUIER FUNCTION CODE
//-----------------------------------------------------------------------------
//
...
//    if ( MemSetUsedLen(RxModBusTcpToken, pUnitelwayInMsg->LEN_BYTE) != OK){
//       #if DEBUG_AT(5)
//  		  (void)sprintf((char *)udp_buf,"NexoModbus Err:0xC7 \n");
//        udp_data_to_send = TRUE;
//       #endif  	
...
//    }
    ...
    PutEventMsg (EVT_SCI_TXCMD, NEX_UnitelwaySci, PROC_NEXO, wSenRequestParam );   
...
//-----------------------------------------------------------------------------
// Procesar respuesta recibida en Unitelway 485 MASTER
//-----------------------------------------------------------------------------
//
void f_NexusUnitelwayRtuMsg (void)
{
  ...
  {                    
    ...
    else
    {
          ...
          PutEventMsg (EVT_UNITELWAY_TXMSG, PROC_UNITELWAY_TCP, PROC_NEXO, CurrentRxToken);
    }
...
void f_NexusUnitelwayRtuErr (void)
{
    ...
    PutEventMsg (EVT_UNITELWAY_ERR, PROC_UNITELWAY_TCP, PROC_NEXO, CurrentRxToken);               
...
void NEX_Init(void)
{
...
#ifdef   TCP_UNITELWAY_MODE    
		NEX_UnitelwaySci = PROC_SCIB;
    SCIB_Init (PROC_NEXO, SCI_FRAME_UNITELWAY, EVT2_SCI_RXMSG, EVT2_SCI_RXERR, EVT2_SCI_TXMSG, EVT2_SCI_TXERR, EVT2_SCI_TXBUSY);
#endif    
   ...
   if (SYS_ConfigParam.DebugLevel>= DBG_LEVEL_3)
  {                    
    	  (void)sprintf((char *)udp_buf,"ModbusTCP-RTU NEX_INIT ScanRate: %d \n",
    	      TMR_BetweenRequest);
        udp_data_to_send = TRUE;
  }
 #endif                        
